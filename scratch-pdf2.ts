...
async function main() {
  ...
  const coverJpegPath = '/home/mostafa/Work/mostakl/realestate/real-estate-back/src/seeders/cover.jpeg';
  const endJpegPath = '/home/mostafa/Work/mostakl/realestate/real-estate-back/src/seeders/end.jpeg';
  
  const coverBase64 = fs.readFileSync(coverJpegPath).toString('base64');
  const endBase64 = fs.readFileSync(endJpegPath).toString('base64');
  
  const coverImage = `data:image/jpeg;base64,${coverBase64}`;
  const endImage = `data:image/jpeg;base64,${endBase64}`;
  ...
  const pdfPath = '/home/mostafa/.gemini/antigravity-cli/brain/abd7cee0-aaca-4551-9e27-6874973ad1bb/report-preview-with-bg.pdf';
  const htmlPath = '/tmp/report-preview2.html';
